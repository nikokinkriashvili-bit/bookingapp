...
- [~] **4.2** ⚙️ Rebooking/maintenance reminders done: migration 014 (`services.reminder_interval_days`, `rebooking_reminders` table, **not yet run**); `src/lib/rebookingReminders.ts` creates a reminder when a job flips to `complete`, keyed to the longest interval among its services; new `/reminders` screen (linked from Home) lists due/upcoming reminders with Send WhatsApp (fires the `rebooking_reminder` seam), Book now (prefills intake via `?plate=`), and Dismiss; per-service interval set in `/settings`. *(Coating-warranty variant F3 rejected by Niko — generic reminders only.)* **Blocked on Niko:** run migration 014; set a reminder interval on a service and confirm a completed job produces a reminder that shows up (needs a real login session, not just the browser preview).
- [ ] **4.3** ⚙️ Public booking link (`/book/<slug>`, BK-01): public route (carve-out in the 1.4 auth gate), business slug, service+slot picker respecting working hours, booking lands as `booked` job. Anti-abuse via Edge Function rate limit. **Prerequisite (F4): `business_closures`** — ✅ **done** (migration 015, `src/lib/closures.ts`, Closures section in `/settings`, closed-day shading on the calendar month grid; `isDateClosed` unit-tested). **The public-booking half is paused for a Niko decision** (see below) — it opens the app's first unauthenticated write path, and its specified anti-abuse mechanism (Edge Function rate limit) lives in Stage 6, which isn't built yet.
  - **Decision (Niko, July 2026): wait — build the public page together with Stage 6** so the Edge Function rate limit ships with it. Do not open the unauthenticated write path before then. F4 (closures) stays done and in use.
- [~] **4.3b** ⚙️ Manual payment recording (F5) done: migration 016 (`payments` table, **not yet run**); `src/lib/payments.ts` with pure `sumPayments`/`outstandingBalance`/`isSettled` helpers (unit-tested) + `recordPayment` that auto-flips a job to `paid` (via `applyStatusChange`, so the `payment_confirmed` seam fires) once payments cover the price; a Payments section on the edit screen shows paid/remaining, lists payments, and records amount + method (cash/transfer/BOG); the dashboard "pending payments" stat now subtracts recorded payments so it shows true remaining balances on part-paid jobs. BOG (6.3) later becomes one more writer to the same table. **Blocked on Niko:** run migration 016.
- [x] **4.4** ❌ **Cut (Niko, July 2026):** deposit collection at booking. Deposits are the detailer's own negotiation with the customer, not something the app enforces or tracks structurally — 4.3b's manual payment recording already covers "record what came in," which is enough.
- [ ] **4.5** ⏸️ **Skipped for now (Niko, July 2026):** capacity limits. Revisit if pilot feedback shows it's actually needed; no definition to build against yet.
- [~] **4.6** ⚙️ Push notification seam + in-app alerts done: migration 020 (`push_tokens` table, **not yet run**); `src/lib/pushTokens.ts` registers each device's Expo push token (best-effort — no-ops on web, needs an EAS project id this app doesn't have yet, so it's a Stage-6-ready seam, not working sends); `src/lib/alerts.ts` pure `computeLowStockAlerts`/`computeJobAlerts` (unit-tested) compute low-stock/overdue/no-show conditions from data already on hand; a new `AlertsBanner` on Home surfaces them in-app (tapping jumps to inventory or the job). **Real background push delivery still needs Stage 6** (Edge Function + a cron to actually send) — this ships the registration half and an honest in-app substitute for now. Customer-facing confirm/reject on WhatsApp messages remains a Stage 6 item (needs WhatsApp itself first). **Blocked on Niko:** run migration 020.
- [~] **4.7** ⚙️ Downloadable PDF receipt done: `expo-print`+`expo-sharing` installed; `src/lib/receipt.ts` (`buildReceiptHtml` pure/unit-tested + `printReceipt`) builds a receipt (business, vehicle, customer, services, condition note, payments, total/paid) from data already on the edit screen; "Download receipt" button shown once a job is `complete`/`paid` — opens the OS print/share sheet (native) or the browser print dialog (web, "Save as PDF"). No migration.
- [~] **4.8** ⚙️ Repeat-last-order shortcut done: vehicle profile shows "Repeat last order" (when it has history) alongside "New order", passing `?plate=&services=id1,id2`; intake preselects those service ids once loaded, silently dropping any archived since. No migration.
- [~] **4.9** ⚙️ Services soft-delete done: migration 019 (`services.archived`, **not yet run**); Settings' remove-service now archives (not deletes) on save, with an "Archived services" list + one-tap Restore; intake fetches only active services; the edit screen fetches all but hides archived ones from the picker unless the job already has them selected (so old bookings keep showing their service name/range). Closes audit finding D3.
...
Every box in Stages 0–3 and 7, plus 4.1–4.3b + 4.6–4.9, plus 6.1–6.3, checked. 4.4 is cut, 4.5 is skipped (both by Niko decision); 6.4 waits for the importer TRD. When that set is green, the platform does end-to-end what the TRD promised: book → work → notify → pay → restock — bilingually, on a phone, in under 30 seconds per booking.
...
| July 2026 | Booking-model pivot, Slice 1 (code done, needs Niko) | Service **price ranges** (migration 017 not yet run) — detailers quote after inspecting; intake price now optional with a suggested-range hint. See CLAUDE.md Confirmed product decisions. |
| July 2026 | Booking-model pivot, Slice 2 (code done, needs Niko) | Per-job **quote flow** (migration 018 not yet run): quote price + description + 3-day expiry + accept/decline on the edit screen; accepting locks the quoted price as the job price; `quote_sent` WhatsApp seam + `quote_token` ready for the Stage 6 public link. Detailer flips the response by hand for now. |
| July 2026 | 4.4 cut, 4.5 skipped (Niko) | Deposit collection cut — pure detailer/customer negotiation, not app-enforced. Capacity limits skipped — no definition yet, revisit post-pilot. |
| July 2026 | Stage 4.9 (code done, needs Niko) | Services soft-delete: migration 019 (`archived` flag, not yet run) — Settings archive/restore, intake/edit filtering. Closes audit D3. |
| July 2026 | Stage 4.8 ✅ | Repeat-last-order: vehicle profile → intake via `?plate=&services=`, no migration needed. |
| July 2026 | Stage 4.7 ✅ | Downloadable PDF receipt: `expo-print`+`expo-sharing`, `buildReceiptHtml` (unit-tested) + OS print/share sheet. No migration. |
| July 2026 | Stage 4.6 (code done, needs Niko) | Push token registration seam (migration 020, not yet run) + in-app Alerts banner on Home (low stock/overdue/no-show), `computeLowStockAlerts`/`computeJobAlerts` unit-tested. Real push send still waits for Stage 6. |
