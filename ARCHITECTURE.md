...
| `/jobs/[id]/edit` | `app/(tabs)/jobs/[id]/edit.tsx` | `jobs` (update), `job_products` (via `consumption.ts`) | Full edit incl. product consumption. |
...
| `services` | 001 | Per-business service catalog (name, duration, price). |
...
| `jobs` | 003, 005 (+`scheduled_end`), 009 (+`assigned_staff_id`) | Core booking record. `status` enum: booked/in_progress/awaiting_collection/complete/paid/cancelled. |
